...
Deno.serve(async (req) => {
    try {
        ...
        doc.text('Einkünfte aus Vermietung und Verpachtung', 20, y);
        y += 15;

        // Grundstücksdaten
        doc.setFontSize(12);
        doc.setFont(undefined, 'bold');
        doc.text('Grundstücksdaten', 20, y);
        y += 8;
        ...
        const grundstuecksdaten = [
            ...
            ['Wohnfläche', `${formData.zeile_10 || 0} m²`]
        ];
        ...
        einnahmen.forEach(([zeile, label, value]) => {
            if (value !== undefined && value !== null) {
                ...
                doc.text(`${Number(value).toLocaleString('de-DE', { minimumFractionDigits: 2 })} €`, 140, y, { align: 'right' });
                y += 6;
            }
        });
        ...
        doc.setFontSize(12);
        doc.setFont(undefined, 'bold');
        doc.text('Zeile 84: Einkünfte aus V+V', 20, y);
        doc.text(`${einkuenfte.toLocaleString('de-DE', { minimumFractionDigits: 2 })} €`, 140, y, { align: 'right' });
