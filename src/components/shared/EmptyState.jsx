...
export default function EmptyState({ icon: Icon, title, description, actionLabel, action }) {
    return (
        <div className="flex flex-col items-center justify-center py-20 px-4">
            ...
            <h3 className="text-lg font-light text-slate-600 mb-2">{title}</h3>
            <p className="text-sm font-extralight text-slate-400 text-center max-w-md mb-8">{description}</p>
            {action && (
                <Button 
                    onClick={action}
